            ...
            if (nodes.getLength() != 0) {
                ...
                for (int i = 0; i < nodes.getLength(); i++) {
                    ...
                    try {
                        ...
                        String ref = OAIUtil.xmlEncode(xmlsiginfo.item(0)
                                .getURI());
                        ...
                        if (ki != null) {
                            ...
                            if (kinfo.containsRetrievalMethod()) {
                                RetrievalMethod m = kinfo
                                        .itemRetrievalMethod(0);
                                ...
                                cert = signature.getKeyInfo()
                                        .getX509Certificate();
                            }

                            if (cert != null) {
                                isVerified = signature
                                        .checkSignatureValue(cert);
                                System.out.println("The XML signature is "
                                        + (isVerified ? "valid (good)"
                                                : "invalid !!!!! (bad)"));
                                if (isVerified) {
                                    sb.append(" status=\"0\"> ");
                                } else {
                                ...
                                sb.append(isVerified ? "valid (good)"
                                        : "invalid !!!!! (bad)");
                            } else {
                                System.out
                                        .println("Did not find a Certificate");
                                PublicKey pk = signature.getKeyInfo()
                                        .getPublicKey();
                                if (pk != null) {
                                    isVerified = signature
                                            .checkSignatureValue(pk);
                                    if (isVerified) {
                                        sb.append(" status=\"0\" >");
                                    } else {
                                        sb.append(" status=\"1\" >");
                ...
                for (int i = 0; i < nodes.getLength(); i++) {
                    ...
                    if (ki != null) {
                        ...
                        if (kinfo.containsRetrievalMethod()) {
                            ...
                            CertificateFactory cf = CertificateFactory
                                    .getInstance("X.509");
                            cert = (X509Certificate) cf.generateCertificate(url
                                    .openStream());
                        } else {
                        ...
                        if (cert != null) {
                            System.out
                                    .println("The XML signature is "
                                            + (signature
                                                    .checkSignatureValue(cert) ? "valid (good)"
                                                    : "invalid !!!!! (bad)"));
                        } else {
                            ...
                            if (pk != null) {
                                System.out
                                        .println("The XML signatur is "
                                                + (signature
                                                        .checkSignatureValue(pk) ? "valid (good)"
                                                        : "invalid !!!!! (bad)"));
                            } else {
                                System.out
                                        .println("Did not find a public key, so I can't check the signature");
