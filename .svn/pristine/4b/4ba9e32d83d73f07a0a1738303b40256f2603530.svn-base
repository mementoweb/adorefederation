...
public class ProxyServlet extends HttpServlet {
	...
	public void init() throws ServletException {
		...
		baseurl = getServletConfig().getServletContext().getInitParameter(
				"baseUrl");
		permurl = getServletConfig().getServletContext().getInitParameter(
				"permurl");
	...
	public String getAuthInfo(String ip) throws IOException {
		String auth = authurl
				+ "?url_ver=Z39.88-2004&rft_id=info:lanl-repo/oppie&"
				+ "svc_id=info:lanl-repo/svc/oppie/auth-getinst&"
				+ "svc_val_fmt=http://oppie.lanl.gov/openurl/auth-getinst.html&svc.ip="
				+ ip;
	...
	public void dispatch(String strurl, HttpServletResponse res)
			throws Exception {
		...
		try {
			...
			//System.out.println("from permalink:"+ huc.getHeaderField("Link"));
			...
			if (code == 200) {
				...
				System.out.println("An error of type " + code
						+ " occurred for:" + strurl);
	...
	public void doGet(HttpServletRequest request, HttpServletResponse res)
		...
		URL _URL = new URL(request.getScheme(), request.getServerName(),
				request.getServerPort(), _file);

		String url = reconstructedURL.toString();

		String _url = _URL.toString();
		// obrezat' service zdes'
		...
		String resolver = baseurl + "/" + service + "?url_ver=Z39.88-2004"
				+ "&rft_id=" + java.net.URLEncoder.encode(id, "UTF8");
		...
		try {
			if (user_agent.indexOf("Mozilla") >= 0) {
				resolver = resolver + "&svc_id=" + service
						+ "&svc.openurl=false" + "&req_id=human" + "&res_id="
						+ java.net.URLEncoder.encode(permurl, "UTF8")
						+ "&svc.access=" + inst + "&svc.sfxbaseurl="
						+ java.net.URLEncoder.encode(sfxurl, "UTF8");
				dispatch(resolver, res);
			} else {
				resolver = resolver + "&svc_id=" + service
						+ "&svc.openurl=false" + "&res_id="
						+ java.net.URLEncoder.encode(permurl, "UTF8")
						+ "&svc.access=" + inst + "&svc.sfxbaseurl="
						+ java.net.URLEncoder.encode(sfxurl, "UTF8");
			...
			mylog.info(formatter.format(new Date()) + "," + url + "," + host
					+ "," + ip);
