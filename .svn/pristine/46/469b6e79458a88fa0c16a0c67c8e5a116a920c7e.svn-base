...
public final class UUID implements java.io.Serializable {
    ...
    public UUID(long _hi, long _lo) {
        ...
        // set multicast bit (so that it there is no chance it will clash with
        // other UUIDs generated based on real IEEE 802 addresses)
    ...
    public int hashCode() {
        ...
        //return new Long(((((long)(new Long(hi).hashCode())) << 32) +
        // ((long)(new Long(lo).hashCode())))).hashCode();
    }
    ...
    public String getNudeId() {
        return (hexDigits(hi >> 32, 4) // time_low: 4 hexOctet (8 hex digits)
                + "-" + hexDigits(hi >> 16, 2) // time_mid: 2 hexOctet (4 hex
                                               // digits)
                + "-" + hexDigits(hi, 2) // time_high_and_version: 2 hexOctet (4
                                         // hex digits)
                + "-" + hexDigits(lo >> 48, 2) // clock_seq_and_reserved: 1
                                               // hexOctet (2 hex digits) &
                                               // clock_seq_low: 1 hexOctet (2
                                               // hex digits)
    ...
    private static String hexDigits(long lVal, int nHexOctets) {
        long tmp = 1L << (nHexOctets * 2 * 4); // e.g., if nHexOctets is 2, tmp
                                               // = (1 0000 0000 0000 0000)b &
                                               // tmp - 1 = (1111 1111 1111
                                               // 1111)b
        long result = lVal & (tmp - 1); // get ride of the uneeded most
                                        // significant bits
        result = tmp | result; // make sure the digit at position (nDigits + 1)
                               // equals 1 (to preserve leading zeroes)
        return Long.toHexString(result).substring(1); // getride ot the digit at
                                                      // position nDigits + 1
    }
    ///////////////////////////////////////////////////
}
