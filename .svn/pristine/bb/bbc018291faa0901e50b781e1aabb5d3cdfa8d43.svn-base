...
public class PubMedProcessor extends DerefProcessor {
    ...
    static Logger log = Logger.getLogger(PubMedProcessor.class.getName());
    ...
    public ProcessInfo processContent(String record, ARCFileWriter arcwriter) {
        ...
        try {
            ...
            String articleBaseUrl = "http://www.pubmedcentral.gov/articlerender.fcgi";
            String pdfBaseUrl = "http://www.pubmedcentral.gov/picrender.fcgi";
            String blobTypeSuffix = "&blobtype=pdf";
            String link = id.replace(articleBaseUrl, pdfBaseUrl).concat(blobTypeSuffix);
            
            // Download bytes
            byte[] deref = util.getBinary(link);
            
            if (new String(deref).contains("<title>PMC error</title>"))
                throw new Exception("Full Text PDF Not Available");
            
            csv.setSourceURI(link);
            String mimetype = "application/pdf";
            String digest = "urn:sha1:" + util.calculateDigest(deref, "SHA1");
            ...
            log.warn("PubMedProcessor:" + e.getMessage());
        }
