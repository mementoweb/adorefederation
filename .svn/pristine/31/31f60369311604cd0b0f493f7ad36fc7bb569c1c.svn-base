...
package gov.lanl.archive.trans.didl.inspec;
...
public class InspecDidl implements DidlTransformer, ResourceParserInterface {
    private AI didWriter;
    private Logger log = Logger.getLogger(InspecDidl.class.getName());
    private static javax.xml.parsers.DocumentBuilderFactory dbf = null;
    private static byte[] xmlDec = new byte[0];
    private static final String COPYRIGHT = "copyright IEE";
    private static final String COLLECTION = "info:sid/library.lanl.gov:inspec";
        ...
        if (rindex.toString().equals("1")) {
            ...
            try {
                ...
                if (!skip) {
                    ...
                    if (arcmimetype.indexOf("inspec+") > 0) {
                        arcmimetype = arcmimetype.replaceAll("inspec\\+", "");
                        type = INSPECXML;
                    }
                    ...
                    if (type == MARCXML) {
                        didWriter.addComponent(
                                ...
                                null,
                                resource);
                    } else if (type == INSPECXML) {
                        didWriter.addComponent(
                                AI.COMPONENT_TYPE.AIXML,
                                arcid,
                                digest,
                                AdoreConstants.INSPEC_XML_FORMATID,
                                created,
                                creator,
                                arcmimetype,
                                null,
    ...
    public Vector process(String oldDidl, String id, ARCFileWriter arcwriter) throws Exception {
        ...
        // Initialize Inspec DIDL Processor
        didWriter = new AI();
