...
public class OpenURLaDORe7SRUService extends XQueryResolverService {
    
    static Logger log = Logger.getLogger(OpenURLaDORe7SRUService.class.getName());
    
    private XQueryProfile profile;
    
    private XQueryResolver resolver;
    
    public OpenURLaDORe7SRUService(ContextObject contextObject) throws OpenURLException, IOException {
        super(contextObject);
    }
    
    public void run(String referent) {
        try {
            bytes = resolver.getResultsAsSRU(referent, profile);
