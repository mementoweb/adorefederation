...
public class OpenURLaDORe7ListService extends XQueryResolverService {
    
    static Logger log = Logger.getLogger(OpenURLaDORe7ListService.class.getName());
    
    private XQueryProfile profile;
    
    private XQueryResolver resolver;
    
    public OpenURLaDORe7ListService(ContextObject contextObject) throws OpenURLException, IOException {
        super(contextObject);
    }
    
    public void run(String referent) {
        try {
            bytes = resolver.getResultsAsList(referent, profile);
