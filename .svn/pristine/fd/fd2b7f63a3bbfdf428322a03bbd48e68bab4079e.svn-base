...
public class SemanticId extends OAIPMHId {

    boolean init = false;
    
    String check = SemanticRegistryProperties.getSemanticValidation();

    public boolean checkId(String id) throws Exception {
        if (!init)
            SemanticRegistryProperties.load(RepoProperties.getProperties());
    ...
    public String getId(String meta) throws Exception {
        try {
            ...
            String ipath = "//sem:identifier";
            XPath xpath = XPath.newInstance(ipath);
            xpath.addNamespace("sem", SemanticRegistryProperties.SEMANTIC_REG_NS);
