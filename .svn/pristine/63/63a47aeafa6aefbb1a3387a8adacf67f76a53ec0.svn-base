...
import gov.lanl.adore.ingestion.SipProcessor;
 ...
 * 
 */
public class SipDIDLProcessor implements ProfileProcessor, FragmentProcessor {
    ...
    public boolean processXMLtape(String sourceFile) {
        ...
        try {
            ...
            // need to change the flow here somewhere -- get the darned sip,
            // then write
    ...
    public void closeWriters() throws IOException {
        ...
        for (String arc : arcs)
            writer.addArcFileID(ARCProperties.getLocalArcFilePrefix()
                    + getFileNameMinusExt(arc));
    ...
    public void processDidl(String metadata, String id, String datestamp)
        ...
        String params = "";
        while (sipDidlMatcher.find()) {
            metadata = sipDidlMatcher.group(1);
            params = sipDidlMatcher.group(2);
        }

        // log.info("DIDL: " + metadata.length());
        // log.info("Param: " + params.length());
        ...
        // process SIP instructions
        SipProcessor processor = new SipProcessor(metadata, paramList
                .toArray(new String[] {}), arcw);
        String output = processor.parseSipDidl();
        ...
        if (didl != null) {
            ...
            if (writer.getSize() >= maxTapeSize) {
                ...
                initWriters(rootTapeName + "-" + formatter.format(tapeCount),
                        null);
            }
        }
    }
