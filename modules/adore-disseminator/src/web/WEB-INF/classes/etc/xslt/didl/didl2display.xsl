          ...
          <td>
            <xsl:choose>
             <xsl:when test="@ref">
                ...
                location: <xsl:element name="a"><xsl:attribute name="class">resource_ref</xsl:attribute><xsl:attribute name="href">$ADORE_DISSEMINATOR_BASE_URL/service?url_ver=Z39.88-2004&amp;svc_id=info:lanl-repo/svc/getDatastream&amp;rft_id=<xsl:value-of select="@ref"/></xsl:attribute>[click to view]</xsl:element>
             </xsl:when>
