...
.newtab{target-new: tab !important;}
-->
</style>
...
<input type="hidden" name="stylesheet" value="permalink"/>
...
<table border="0">
 ...
 <xsl:template match="result">
  <xsl:apply-templates>
    <xsl:sort select="str[@name='displayTitle']" />
  </xsl:apply-templates>
</xsl:template>
   ...
   <div class="doc">
      <table width="100%">
        ...
        <font size="-1">
          ...
          <br/>
               <xsl:variable name="recID" select="str[@name='recID']" />
        <xsl:element name="A">
        <xsl:attribute name="href">http://flasher.lanl.gov:8080/adore-permalink/object?rft_id=<xsl:value-of select="$recID"/></xsl:attribute>
        <xsl:attribute name="class">newtab</xsl:attribute>  
        <!--<xsl:attribute name="onClick">javascript:goURL('http://flasher.lanl.gov:8080/adore-permalink/object?rft_id=<xsl:value-of select="$recID"/>');</xsl:attribute>-->  
        http://flasher.lanl.gov:8080/adore-permalink/object?rft_id=<xsl:value-of select="$recID"/></xsl:element> 
        </font>
        </span>
        <br/><br/>
             </td>
