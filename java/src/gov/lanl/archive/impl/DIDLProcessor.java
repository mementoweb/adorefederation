...
import gov.lanl.archive.trans.didl.TapeParser;
...
/**
 * aDORe Archive DIDL Content Pre-processor - Expects a Source Array of 
 * XMLtape file objects containing DIDL content in all taperecords. Uses
 * an underlying DidlTransformer implem. to perform the DIDL conversion.
 *
 */
public class DIDLProcessor implements ProfileProcessor, FragmentProcessor {
    ...
    public void runIt() throws Exception {
        ...
        this.setConverterClass(archiveProfile.getCollectionConverterPlugin());
    ...
    /**
     * Process source XMLtape, parse & update DIDL DO's, write datastreams
     * to arc file. 
     * @param sourceFile
    ...
    public boolean processXMLtape(String sourceFile) {
        ...
        try {
            ...
            // Parse XMLTape to Perform Transformation
            FileInputStream fis = new FileInputStream(new File(sourceFile));
            TapeParser handler = new TapeParser(fis, (FragmentProcessor) this);
    ...
    public void processDidl(String metadata, String id, String datestamp)
        ...
        Vector data = transformer.process(metadata, id, arcw);
    ...
    private void setConverterClass(String conClass) throws ArchiveException {
        try {
            this.transformer = (DidlTransformer) Class.forName(conClass).newInstance();
            transformer.setProperties(archiveConfig.getProperties());
        } catch (InstantiationException e) {
            throw new ArchiveException("InstantiationException for " + conClass, e);
        } catch (IllegalAccessException e) {
            throw new ArchiveException("IllegalAccessException for" + conClass, e);
        } catch (ClassNotFoundException e) {
            throw new ArchiveException("ClassNotFoundException for " + conClass, e);
        } catch (Exception e) {
