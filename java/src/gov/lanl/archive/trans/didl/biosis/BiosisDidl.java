...
package gov.lanl.archive.trans.didl.biosis;
...
public class BiosisDidl implements DidlTransformer, ResourceParserInterface {
    private AI didWriter;
    private Logger log = Logger.getLogger(BiosisDidl.class.getName());
    private static javax.xml.parsers.DocumentBuilderFactory dbf = null;
    private static byte[] xmlDec = new byte[0];
    private static final String COPYRIGHT = "copyright BIOSIS";
    private static final String COLLECTION = "info:sid/library.lanl.gov:biosis";
        ...
        if (rindex.toString().equals("1")) {
            ...
            try {
                ...
                if (!skip) {
                    ...
                    if (arcmimetype.indexOf("biosis+") > 0) {
                        arcmimetype = arcmimetype.replaceAll("biosis\\+", "");
                        type = BIOSISXML;
                    }
