...
public class CreateXMLSig {
     ...
     * @throws Exception
     */
    public CreateXMLSig(String keystoreType, String keystoreFile,
        ...
        // Get the private key that will be used to sign the document
        privk = (PrivateKey) keyStore.getKey(privateKeyAlias, privateKeyPass
                .toCharArray());
    ...
    public String signDidl(Document doc) throws Exception {
        ...
        Element didl = (Element) doc.getElementsByTagNameNS(DIDL_NS, "DIDL")
                .item(0);
        Element item = (Element) doc.getElementsByTagNameNS(DIDL_NS, "Item")
                .item(0);
        ...
        String[][] filters = {
                { XPath2FilterContainer.INTERSECT, "here()/ancestor::*[7]" },
                { XPath2FilterContainer.SUBTRACT, "here()/ancestor::dsig:Signature[1]" } };
        ...
        xmlSig.getKeyInfo().add(new RetrievalMethod(doc, certificateURL, null,
    ...
    public String signRef(String ref, String digest) throws Exception {
        ...
        signature.getKeyInfo().add(
                new RetrievalMethod(doc, certificateURL, null,
                        "http://www.w3.org/2000/09/xmldsig#X509Data"));
    ...
    public String signGzipRef(String ref, String digest) throws Exception {
        ...
        signature.getKeyInfo().add(new RetrievalMethod(doc, certificateURL, null,
                        "http://www.w3.org/2000/09/xmldsig#X509Data"));
        doc.appendChild(signature.getElement());
        if (digest == null) {
            signature.sign(privk);
     ...
     * @throws Exception
     */
    public String signXMLinline(String refurl, String fragment)
            throws Exception {
    ...
    private String signXML(String refurl, String digest) throws Exception {
        ...
        transforms.addTransform(Transforms.TRANSFORM_C14N_EXCL_OMIT_COMMENTS);
        signature
                .addDocument(refurl, transforms, Constants.ALGO_ID_DIGEST_SHA1);
        signature.getKeyInfo().add(
                new RetrievalMethod(doc, certificateURL, null,
                        "http://www.w3.org/2000/09/xmldsig#X509Data"));
        doc.appendChild(signature.getElement());
    ...
    public String calculateDigest(byte[] streamBytes, String algoritm)
            throws NoSuchAlgorithmException {
    ...
    public void setDigestValueElement(String base64codedValue)
            throws XMLSecurityException {
        Reference ref = signature.getSignedInfo().item(0);

        Element digestValueElement = XMLUtils.selectDsNode(ref.getElement()
                .getFirstChild(), Constants._TAG_DIGESTVALUE, 0);
    ...
    public void setSignatureValueElement(byte[] bytes) {
        ...
        Element signatureValueElem = XMLUtils
                .selectDsNode(signature.getElement().getFirstChild(),
     ...
     */
    public void sign(Key signingKey, String digest)
        ...
        try {
            ...
            SignatureAlgorithm sa = new SignatureAlgorithm(
                    signatureMethodElement, signature.getBaseURI());
            ...
            OutputStream so = new BufferedOutputStream(new SignerOutputStream(
                    sa));
            try {
                so.close();
            } catch (IOException e) {
    ...
    public static void main(String args[]) throws Exception {
        ...
        CreateXMLSig c = new CreateXMLSig(keystoreType, keystoreFile,
                keystorePass, privateKeyAlias, privateKeyPass, certificateURL);
