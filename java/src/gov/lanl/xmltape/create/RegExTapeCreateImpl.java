...
public class RegExTapeCreateImpl implements TapeCreateInterface {
    ...
    private void init() throws Exception {
        if (props == null)
            throw new Exception("Properties file must be initialized");
        ...
        if (recordmatch != null && recordmatch.trim().length() > 0 )
            recordmatch_pattern = Pattern.compile(recordmatch, Pattern.DOTALL);

        if (identifier_regex != null && identifier_regex.equals("AUTO"))
            identifier_implicit = true;
        else if (identifier_regex != null && identifier_regex.trim().length() > 0)
            identifier_regex_pattern = Pattern.compile(identifier_regex);

        if (datestamp_regex != null && datestamp_regex.equals("AUTO"))
            datestamp_implicit = true;
        else if (datestamp_regex != null && datestamp_regex.trim().length() > 0)
            datestamp_regex_pattern = Pattern.compile(datestamp_regex);

        if (meta_regex != null && meta_regex.trim().length() > 0)
            meta_regex_pattern = Pattern.compile(meta_regex);
    ...
    public int writeRecords(InputStreamReader reader, SingleTapeWriter tapewriter) throws Exception {
        // Initialize the RegEx Criteria
        if (!initialized)
            init();
        // Initialize the buffer
        char[] buffer = new char[INITIAL_BUFFER_SIZE];
        int count = 0;
        // Read file into charbuffer
    ...
    private TapeRecord buildRecord(String record) throws TapeException {
        ...
        if (datestamp_regex_pattern != null && !datestamp_implicit) {
            ...
            if (datestamp_regex_matcher.find()) {
                ...
                throw new TapeException("no datestamp match found for record");
            
            // Normalize Datastamp to OAI UTC
            try {
        ...
        if (identifier_regex_pattern != null && !identifier_implicit) {
            ...
            if (identifier_regex_matcher.find()) {
                ...
                throw new TapeException("no identifier match found for record");
            
            if (props.getRecordIdentifierPrefix() != null)
            	identifier = props.getRecordIdentifierPrefix() + identifier;
            
            if (props.getRecordIdentifierSuffix() != null) {
            	if (props.getRecordIdentifierSuffix().contains("${profile.record.datestamp}"))
            	    identifier = identifier + props.getRecordIdentifierSuffix().replace("${profile.record.datestamp}", datestamp);
            	else
            		identifier = identifier + props.getRecordIdentifierSuffix();
            }
        ...
        if (meta_regex_pattern != null) {
            ...
            if (meta_regex_matcher.find()) {
                ...
                throw new TapeException("no metadata match found for record");
        } else
            meta = record;
