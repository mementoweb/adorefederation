 ...
 * @author Ryan Chute
 */
public class OpenURLAtomFeedService implements Service {

	private ResourceReader reader;
	private static final String svc = "info:lanl-repo/svc/atom";
	 ...
	 */
	public OpenURLAtomFeedService(OpenURLConfig openURLConfig, ClassConfig classConfig) throws ResolverException {
            String propFile;
            try {
                propFile = classConfig.getArg("props");
                Properties props = PropertiesUtil.loadConfigByClasspath(propFile);
                this.reader = new ResourceReader(props);
                this.pidResolver = props.getProperty("adore-resource.PIDResolver");
	...
	public OpenURLResponse resolve(ServiceType serviceType,
		...
		if (kev.containsKey("until"))
			until = kev.get("until");
		if (kev.containsKey("index"))
			index = Integer.parseInt(kev.get("index"));
		if (kev.containsKey("cnt"))
			perPage = Integer.parseInt(kev.get("cnt"));

		responseFormat = " application/atom+xml; charset=UTF-8; type=feed";
		try {
			OpenURLAtomFeedBuilder feed = new OpenURLAtomFeedBuilder(resolver, pidResolver);
			feed.setIndex(index);
			feed.setPerPage(perPage);
			InputStream is = listIdentifiers(id, from, until);
			BufferedReader br = new BufferedReader(new InputStreamReader(is));
			String row = "";
			for (int line = index; true; line++) {
				row = br.readLine();
				if (row == null)
					break;
			...
			response = new ByteArrayInputStream(feed.getFeed().getBytes());
			br.close();
			is.close();
		} catch (IdentifierNotFoundException ex) {
			response = new ByteArrayInputStream((" id doesn't exist " + ex
					.getMessage()).getBytes("UTF-8"));
			responseFormat = "text/plain";
		} catch (ResolverException ex) {
			response = new ByteArrayInputStream((" service exception " + ex
					.getMessage()).getBytes("UTF-8"));
			responseFormat = "text/plain";
		} catch (IOException e) {
			response = new ByteArrayInputStream((" service exception " + e
					.getMessage()).getBytes("UTF-8"));
			responseFormat = "text/plain";
		}

		return new OpenURLResponse(HttpServletResponse.SC_OK, responseFormat,
				response);
	}

	public InputStream listIdentifiers(String repoId, String from, String until)
			throws ResolverException, IdentifierNotFoundException {
		InputStream is;
		try {
			is = reader.listIdentifiers(repoId, from, until);
		} catch (ResourceException e) {
			throw new ResolverException("Error attempting to obtain record: "
					+ e.getMessage());
		}
		if (is == null)
			throw new IdentifierNotFoundException(
					"Unable to resolve identifier: " + repoId);
		return is;
	}
				...
				if (kev.getFieldMap().size() > 0) {
					...
					if (kev.getFieldMap().containsKey("svc.until")
						...
						map.put("until", ((String[]) kev.getFieldMap().get(
								"svc.until"))[0]);
					if (kev.getFieldMap().containsKey("svc.index")
							&& ((String[]) kev.getFieldMap().get("svc.index"))[0] != "")
						map.put("index", ((String[]) kev.getFieldMap().get(
								"svc.index"))[0]);
					if (kev.getFieldMap().containsKey("svc.cnt")
							&& ((String[]) kev.getFieldMap().get("svc.cnt"))[0] != "")
						map.put("cnt", ((String[]) kev.getFieldMap().get(
								"svc.cnt"))[0]);
				}
			}
		}
