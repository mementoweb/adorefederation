...
public class ExtractCDX {
    ...
    public static void main(String[] argv) {
        ...
        for (int i = 0; i < argv.length; i++) {
            ...
            } else if (argv[i].endsWith(".arc") || argv[i].endsWith(".ARC")
                    || argv[i].endsWith(".arc.gz")
                    || argv[i].endsWith(".ARC.gz")) {
    ...
    private static void printHelp() {
        System.err
                .println("Usage: java dk.netarkivet.ArcUtil.ExtractCDX [-f fields] <files>");
    ...
    public static void extractFromArc(String s) throws Exception {
        ...
        if (fields == null)
            fields = (is_compressed ? "AebmngV" : "Aebmngv");
        ...
        try {
            ...
            try {
                do {
                    ARCInputStream in = new ARCInputStream(file, is_compressed,
                            offset);
                    ...
                    fieldsread.put(is_compressed ? "V" : "v", Long
                            .toString(offset));
                    fieldsread.put("g", f.getName());
                    for (int i = 0; i < fieldarray.length; i++) {
                        pw.write((i > 0 ? "\t" : "")
                                + fieldsread.get(fieldarray[i]));
                    }
                    pw.println();
                    //pw.flush();

                    //writeFields(pw,fieldsread, fieldarray);
                    in.readAll();
                    offset = file.getFilePointer();
                } while (offset < f.length());
            } finally {
                pw.flush(); //this flushing is extra important see API
                // this PrintWriter never throws io exception by themself!!!
                if (pw.checkError()) {
    ...
    public static String[] parseFields(String fieldspec) {
        ...
        for (int i = 0; i < chars.length; i++) {
            ...
            if (!legalFields.containsKey(strings[i])) {
                System.err.println("Illegal field key " + strings[i]
                        + ". Legal keys are:");
                for (Iterator it = legalFields.entrySet().iterator(); it
                        .hasNext();) {
                    Map.Entry entry = (Map.Entry) it.next();
    ...
    private static void writeFields(PrintWriter pw, Hashtable fieldsread,
            String[] fields) {
        for (int i = 0; i < fields.length; i++) {
    ...
    static final Matcher headmatch = Pattern.compile(
            "(\\S+)\\s+([0-9.]+)\\s+(\\d+)\\s+(\\S+)\\s+(\\d+)").matcher("");
