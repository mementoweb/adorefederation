...
public class SRUSearchRetrieveResponseParser extends DefaultHandler implements SRUConstants {
     ...
     */
    public void parse(SRUSearchRetrieveResponse sru, InputStream stream) throws Exception {
        this.sru=sru;
     ...
     * If SRU or DC namespace is found in the uri, the element is flagged for extraction.
     */
    public void startElement(
        ...
        try {
            if (SRU_NAMESPACE.equals(uri) && localName.equals(TAG_SRU_SRR_VERSION))
                ...
                inline = true;
            else if (DC_NAMESPACE.equals(uri) && localName.equals(DC_TAG)) {
                if (dc != null) {
                   sru.addRecord(dc);
                   dc = new SRUDC();
                }
            ...
            } else if (DC_NAMESPACE.equals(uri))
                inline = true;
        } catch (Exception ex){
