...
package gov.lanl.didlwriter.profile.aiext;
...
/**
 * AI Extension (MarcXML, Source XML, PDF)
 * @author rchute
 */
public class AIEXT {
    ...
    private AdoreComponent aiPdfCom;
    ...
    public AdoreComponent getAIPdfCom() {
        return aiPdfCom;
    }
     ...
     */
    public void addComponent(AIEXT.COMPONENT_TYPE type, String componentID,
        ...
        if ((type == AIEXT.COMPONENT_TYPE.MARCXML) && (content != null)) {
        ...
        if (type == COMPONENT_TYPE.MARCXML) {
            ...
            diadm.getSemantic().add(AdoreConstants.BIBLIOGRAPHIC_SEMANTICID);
            aiXMLCom = com;
        } else if (type == COMPONENT_TYPE.PDF) {
            if (creator != null && creator.length() > 0)
                diadm.setCreator(creator);
    ...
    public AdoreDIDL create() throws LANLDIDLException {
        try {
            ...
            diadm.setFormat(AdoreConstants.ITEM_AI_EXT_FORMATID);
            if (collection != null)
            ...
            rootItem.getComs().add(aiPdfCom);

            didl.setItem(rootItem);
            return didl;
        } catch (Exception ex) {
            throw new LANLDIDLException("error in create AIEXT record",
                    ex);
        }
    ...
    public void parse(java.io.InputStream stream) throws LANLDIDLException {
        try {
            ...
            aiPdfCom = didl.getItem().getComs().get(2);
        } catch (Exception ex) {
            throw new LANLDIDLException("error in parse AIEXT", ex);
        }
    }
